...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("mac.attack_mode",
        bpo::value<bool>(&args->stack.mac.attack_mode)->default_value(false),
        "Enable attack mode (flooding UE)")

    ("mac.attack_prach_period_ms",
        bpo::value<uint32_t>(&args->stack.mac.attack_prach_period_ms)->default_value(20),
        "PRACH transmission period in attack mode (ms)")
