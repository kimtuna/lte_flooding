...
bool mac::init(phy_interface_mac_lte* phy, rlc_interface_mac* rlc, rrc_interface_mac* rrc)
{
  ...
  ra_procedure.init(phy_h, rrc, &uernti, &timer_alignment, &mux_unit, &task_sched, this, &attack_ue_instance);
  sr_procedure.init(&ra_procedure, phy_h, rrc);
  
  // Initialize attack UE instance
  // Note: rrc_h is rrc_interface_mac*, we need to cast to rrc* for direct method access
  // For now, we'll pass nullptr and use rrc_interface_mac methods instead
...
void mac::tb_decoded(uint32_t cc_idx, mac_grant_dl_t grant, bool ack[SRSRAN_MAX_CODEWORDS])
{
  if (grant.is_pdcch_order) {
    ...
    if (ack[0]) {
      ...
      // 공격 모드일 때는 attack_ue에도 RAR 정보 전달
      // NOTE: 실제 RAR 파싱은 ra_procedure에서 수행되므로,
      // attack_ue는 ra_procedure의 콜백을 통해 RAR 정보를 받아야 함
      // 또는 ra_procedure에 attack_ue 포인터를 전달하여 직접 호출
    }
...
void mac::set_attack_prach_period_ms(uint32_t period_ms)
{
  attack_ue_instance.set_prach_period_ms(period_ms);
}
