...
srsran::proc_outcome_t rrc::connection_setup_proc::react(const bool& config_complete)
{
  ...
  // 공격 모드일 때는 Msg5 (RRCConnectionSetupComplete) 전송하지 않음
  if (rrc_ptr->mac && rrc_ptr->mac->is_attack_mode_enabled()) {
    rrc_ptr->logger.info("Attack mode: Blocking RRCConnectionSetupComplete transmission (Msg5 blocked)");
    return proc_outcome_t::error; // 에러로 처리하여 절차 종료
  }
