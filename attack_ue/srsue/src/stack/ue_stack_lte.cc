...
int ue_stack_lte::init(const stack_args_t& args_)
{
  ...
  // Configure attack mode if enabled
  if (args.stack.mac.attack_mode) {
    mac.set_attack_mode(true);
    mac.set_attack_prach_period_ms(args.stack.mac.attack_prach_period_ms);
  }
